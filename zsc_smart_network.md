...
为了保持与以太坊共识协议（包括即将到来的升级）的兼容性，ZSC智能网络上有一个专用于ZSC权益质押的模块。 它将接受ZTB 持有者的ZSC权益质押，享受质押挖矿获得ZTB奖励。
...
对于ZSC智能网络的未来，很难做出一个定论，因为它从未停止进化。目前只是ZSC智能网络发展的一个阶段。以下是一些值得关注的主题，以促进社区更好地获得更多的可用性和可扩展性：

1. 为不同的业务用例添加不同的数字资产模型,链上更丰富的金融模型。

2. 实现将更多的数据源，特别是 DEX 市场数据，抓取更多的ZSC链数据信息。

3. 提供兼容以太坊及其未来升级，以及其他区块链的接口。兼容更多的区块链底层网络。

4. 改进钱包和区块链客户端的可用性,优化可使用性。
